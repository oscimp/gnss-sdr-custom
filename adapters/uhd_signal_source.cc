/*!
 * derived from uhd_signal_source.cc (Javier Arribas, 2012)
 */
    ...
    std::shared_ptr<Concurrent_Queue<pmt::pmt_t>> queue) : role_(role), in_stream_(in_stream), out_stream_(out_stream), queue_(std::move(queue))
{
    ...
    if (jmf_ !=0) 
         {printf("JMF %d\n",jmf_);
          RF_channels_=jmf_; // JMF : override RF_channels in this block only => N inputs and RF_chan=1 output
